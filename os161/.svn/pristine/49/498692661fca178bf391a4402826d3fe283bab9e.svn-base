       ...
       unsigned long catnumber)
{
	...
	while (iteration < 4) {
		// use semaphore to protect dishes availability checking
		P(mutex);
		...
		// if it is the first cat starts eating, hold the semaphore between cat and mouse
		if(catsCount == 1)
			P(cats_or_mice);
		...
		// use semaphore to protect changing parameters
		P(mutex);
		// decrease the number of cats
		catsCount--;

		// if all cats finish eating, free the semaphore between cat and mouse
		if (catsCount == 0)
			V(cats_or_mice);
         ...
         unsigned long mousenumber)
{
	...
	while (iteration < 4) {
		...
		// if it is the first mouse starts eating, hold the semaphore between cat and mouse
		if(miceCount == 1)
			P(cats_or_mice);
		...
		// if all mice finish eating, free the semaphore between cat and mouse
		if (miceCount == 0)
			V(cats_or_mice);
