...
/*
 * Command for modifying the debug flags.
 */
static
int
cmd_dbflags(int nargs, char **args) {
        // added lines to parse operations for dbflags
        // check if there is right number of arguments from user input
        if (nargs != 3) {
                kprintf("Usage: df nr on/off\n");
        } else {
                // cast the flag number into integer
                int i = atoi(args[1]);

                //check if the last argument is on or off
                if (strcmp(args[2], "on") != 0 && strcmp(args[2], "off") != 0) {
                        kprintf("Usage: df nr on/off\n");
                }

                //check if the flag number within the valid range
                else if ((i > 12) || (i < 1)) {
                        kprintf("Usage: df nr on/off\n");
                }
...
static const char *opsmenu[] = {
	"[s]       Shell                     ",
	"[p]       Other program             ",
        //added option "dbflags" in opsmenu
	"[dbflags] Debug flags               ", 
